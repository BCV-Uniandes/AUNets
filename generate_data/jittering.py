...
def rescale_rect(img, rect_img, new_img):
    ...
    rect_norm = [
        rect_img[0] / imsz[1], rect_img[1] / imsz[0], rect_img[2] / imsz[1],
        rect_img[3] / imsz[0]
    ]
    imsz_new = np.array(new_img.shape[:2]).astype(np.float32)
    new_rect = np.array([
        rect_norm[0] * imsz_new[1], rect_norm[1] * imsz_new[0],
        rect_norm[2] * imsz_new[1], rect_norm[3] * imsz_new[0]
    ]).astype(np.uint16)
    return new_rect
...
def jitter_points(facePoints, max_values, mode):
    ...
    new_face = [
        int(max(new_face[0], 0)),
        int(max(new_face[1], 0)),
        int(min(new_face[2], max_values[1])),
        int(min(new_face[3], max_values[0]))
    ]
    return new_face
