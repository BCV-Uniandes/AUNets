...
class Get_Faces():
    ...
    def from_file(self, img_file, BBOX=False):
        ...
        # bbox_jit = [max(0,bbox[0]-img_jit[0]), \
        #            min(img_shape[0], bbox[1]+img_jit[0]), \
        #             bbox[2], bbox[3]]
...
if __name__ == '__main__':
    ...
    algined_size_root = os.path.join(folder_root,
                                     'Faces_aligned_{}'.format(args.img_size))
    faces_size_root = os.path.join(folder_root,
                                   'Faces_{}'.format(args.img_size))
    ...
    if not os.path.isdir(algined_size_root):
    ...
    for org_file in tqdm.tqdm(
            org_files, total=len(org_files), desc='Extracting Faces'):
        ...
        if not os.path.isdir(face_dir.replace(face_root, algined_root)):
            os.makedirs(face_dir.replace(face_root, algined_root))

        if os.path.isfile(face_file) and os.path.isfile(
                bbox_file):  # and os.stat(bbox_file).st_size==0:
            continue
        ...
        if args.OF:
            ...
            if not os.path.isdir(face_of_dir):
                os.makedirs(face_of_dir)

            if not os.path.isfile(of_face_file):  # or True:
                try:
                    of_face = imageio.imread(
                        of_file)[bbox[0]:bbox[1], bbox[2]:bbox[3]]
                    imageio.imwrite(of_face_file, of_face)
                except BaseException:
                    continue

    for org_file in tqdm.tqdm(
            org_files, desc='Resizing Faces to %d' % (args.img_size)):
        ...
        if args.OF:
            ...
            if os.path.isfile(
                    rs_file_of) and not os.path.isfile(resize_file_of):
